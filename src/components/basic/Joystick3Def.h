...
/**
 * @file Joystick3Def.h
 * @brief Définition du composant Joystick 3 axes
 *
 * Joystick avec 3 axes analogiques (X, Y et Z).
...
/**
 * @brief Configuration spécifique au joystick 3 axes
 */
struct Joystick3Config {
    uint8_t filter_intensity;  // Intensité du filtrage (1-10)

    // Seuils axe X
    uint16_t joyXMin, joyXZeroMin, joyXZeroMax, joyXMax;
    // Seuils axe Y
    uint16_t joyYMin, joyYZeroMin, joyYZeroMax, joyYMax;
    // Seuils axe Z
    uint16_t joyZMin, joyZZeroMin, joyZZeroMax, joyZMax;

    // Inversion par axe
    bool invertX, invertY, invertZ;

    // Configuration MIDI par axe
    MidiMessageType xMsgType, yMsgType, zMsgType;
    uint8_t xMidiParam, yMidiParam, zMidiParam;
    uint8_t xMidiChannel, yMidiChannel, zMidiChannel;
    ...
    Joystick3Config()
        ...
        , joyZMin(200), joyZZeroMin(1900), joyZZeroMax(2100), joyZMax(4000)
        , invertX(false), invertY(false), invertZ(false)
        , xMsgType(MidiMessageType::CONTROL_CHANGE), yMsgType(MidiMessageType::CONTROL_CHANGE), zMsgType(MidiMessageType::CONTROL_CHANGE)
...
/**
 * @brief Définition complète du Joystick 3 axes
 */
struct Joystick3 {
    // Identifiants
    static constexpr const char* ID = "joystick3";
    static constexpr const char* DISPLAY_NAME = "Joystick 3 axes";
    static constexpr const char* FAMILY_NAME = "Basic";

    // Configuration
    static constexpr ComponentFamily FAMILY = ComponentFamily::BASIC;
    static constexpr ComponentType TYPE = ComponentType::JOYSTICK3;
    ...
    static ComponentDefinition createDefinition() {
        static constexpr FormFieldDef FF[] = {
            ...
            // Seuils axe Z
            makeNumberField("zMin", "Z Min", 0, 4095, "200", 1, 100, "f"),
            makeNumberField("zZeroMin", "Z Zero Min", 0, 4095, "1900", 1, 100, "f"),
            makeNumberField("zZeroMax", "Z Zero Max", 0, 4095, "2100", 1, 100, "f"),
            makeNumberField("zMax", "Z Max", 0, 4095, "4000", 1, 100, "f"),
            // Inversion d'axes
            ...
            makeSelectField("invertY", "Inverser axe Y",
                "[{\"value\":\"0\",\"label\":\"Normal\"},{\"value\":\"1\",\"label\":\"Inversé\"}]", "0", "r"),
            makeSelectField("invertZ", "Inverser axe Z",
                "[{\"value\":\"0\",\"label\":\"Normal\"},{\"value\":\"1\",\"label\":\"Inversé\"}]", "0", "r"),
            // Filtrage
            makeNumberFieldWithHint("filterIntensity", "Intensité filtrage (1-10)", 1, 10, "5", "1=rapide, 10=stable", 60, "r"),
        };
        /* cc + range pour axe (dependsOnRole null) : params partagés X/Y/Z */
        ...
        static constexpr MidiMessageDef MM[] = {
            ...
            // Axe Z
            {"cc",         "Control Change (Axe Z)", "CC#{cc}",          "z", 3, CC_RANGE,           3},
            {"pitchbend",  "Pitch Bend (Axe Z)",     "Pitch Bend",       "z", 1, PARAM_CHANNEL_ONLY, 1},
            {"aftertouch", "Aftertouch (Axe Z)",     "Aftertouch",       "z", 1, PARAM_CHANNEL_ONLY, 1},
            {"notesweep",  "Note (balayage) (Axe Z)","Note {note} scan", "z", 4, PARAM_NOTESWEEP,    4},
        };
        /* Pins additionnelles pour axes Y et Z */
        static constexpr AdditionalPinDef AP[] = {
            {"joyYPin", "Pin axe Y", PinType::PIN_ANALOG, false, 255},
            {"joyZPin", "Pin axe Z", PinType::PIN_ANALOG, false, 255},
        };
        return makeFlashDef(ID, DISPLAY_NAME, "cardJoystick3", FAMILY, FAMILY_NAME, TYPE, PIN_TYPE,
                            SUPPORTS_MIDI, SUPPORTS_OSC, IMPLEMENTED, FF, 15, MM, 12,
                            nullptr, AP, 2);
    }
};
