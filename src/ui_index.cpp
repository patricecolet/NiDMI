 ...
 * { margin: 0; padding: 0; box-sizing: border-box; }
 ...
 .digital { background:#3B82F6; } 
 .analog { background:#EC4899; } 
 .i2c { background:#10B981; } 
 .uart { background:#6B7280; } 
 .spi { background:#8B5CF6; } 
 .power { background:#EF4444; } 
 .gnd { background:#000000; } 
...
<body>
 ...
 <form id="mdns">
 <div class="form-group">
 <label for="mdnsName">Nom du serveur</label>
 <input type="text" id="mdnsName" placeholder="esp32rtpmidi" required>
 <div class="hint">
 <small>Ce nom sera utilisé pour l'AP Wi-Fi et l'accès via http://nom.local</small>
 </div>
 </div>
 <button type="submit">Enregistrer</button>
 <div class="hint" id="mdnsMsg"></div>
 </form>
 </div>
 ...
 if(data.type === 'rtp_status') {
 // Mettre à jour la checkbox RTP-MIDI
 ...
 console.log('RTP-MIDI status updated via WebSocket:', data);
 }
 } catch(e) {
 ...
 loadStatus();
 loadMdnsStatus(); // Charger l'état mDNS au démarrage
 connectWebSocket(); // WebSocket pour les mises à jour en temps réel
 loadCaps();
 drawBoardPins();
 ...
 $('#mdns').onsubmit = async (ev) => {
 ev.preventDefault();
 $('#mdnsMsg').textContent = 'Enregistrement...';
 const res = await fetch('/api/mdns', {
 method: 'POST',
 headers: {'Content-Type': 'application/x-www-form-urlencoded'},
 body: 'name=' + encodeURIComponent($('#mdnsName').value)
 });
 $('#mdnsMsg').textContent = res.ok ? 'Nom du serveur enregistré - Redémarrage nécessaire' : 'Erreur ' + res.status;
 };
