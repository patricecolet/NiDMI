 ...
 *{margin:0;padding:0;box-sizing:border-box}
 ...
 .item:hover{background:var(--bg)}.item.analog{border-left-color:var(--ca)}.item.digital{border-left-color:var(--cd)}.item.i2c{border-left-color:var(--ci)}.item.spi{border-left-color:var(--cs)}.item.uart{border-left-color:var(--cu)}.item.touch{border-left-color:#F59E0B}
 ...
 // L'erreur est normale : le serveur redémarre
 ...
 const mk=(x,y,w,h,fill,stroke,label,clk=true,small=false)=>{ const g=document.createElementNS('http://www.w3.org/2000/svg','g'); const r=document.createElementNS('http://www.w3.org/2000/svg','rect'); r.setAttribute('x',x); r.setAttribute('y',y); r.setAttribute('width',w); r.setAttribute('height',h); r.setAttribute('rx','4'); r.setAttribute('fill',fill); r.setAttribute('stroke',stroke); g.appendChild(r); const t=document.createElementNS('http://www.w3.org/2000/svg','text'); t.setAttribute('x',x+w/2); t.setAttribute('y',y+h/2+1); t.setAttribute('text-anchor','middle'); t.setAttribute('class','svg-t'); if(small)t.setAttribute('style','font-size:7px'); t.textContent=label; g.appendChild(t); if(clk&&label){ g.style.cursor='pointer'; r.dataset.label=label; prect[label]=r; r.addEventListener('click',()=>{ if(window._selRect) window._selRect.classList.remove('selectedSquare'); window._selRect=r; r.classList.add('selectedSquare'); cur=label; $('#selPin').textContent=label; handlePinClick(label); updFunc(label); if(pcfg[cur]) applyCfg(pcfg[cur]); }); } return g; };
 const getPinColor=(label)=>{ if(['TX','RX'].includes(label))return FC.UART; if(label.startsWith('A'))return FC.ANALOG; if(['SDA','SCL'].includes(label))return FC.I2C; if(['MOSI','MISO','SCK'].includes(label))return FC.SPI; return FC.DIGITAL; };
 ...
 if(isS3){
 const left3=(row,busLbl,adcLbl,dLbl)=>{ const y=30+row*RH; const f=document.createDocumentFragment(); if(busLbl){ f.appendChild(mk(COL.c1,y-10,W,H,getPinColor(busLbl),'#9ca3af',busLbl,true,isSmall(busLbl))); }else{ f.appendChild(mk(COL.c1,y-10,W,H,'#9ca3af','#9ca3af','',false)); } if(adcLbl){ f.appendChild(mk(COL.c2,y-10,W,H,FC.ANALOG,'#9ca3af',adcLbl,true)); }else{ f.appendChild(mk(COL.c2,y-10,W,H,'#9ca3af','#9ca3af','',false)); } f.appendChild(mk(COL.c3,y-10,W,H,FC.DIGITAL,'#9ca3af',dLbl,true)); L.appendChild(f); };
 const right3=(row,dLbl,adcLbl,busLbl)=>{ const y=30+row*RH; const f=document.createDocumentFragment(); f.appendChild(mk(COL.c4,y-10,W,H,FC.DIGITAL,'#9ca3af',dLbl,true)); if(adcLbl){ f.appendChild(mk(COL.c5,y-10,W,H,FC.ANALOG,'#9ca3af',adcLbl,true)); }else{ f.appendChild(mk(COL.c5,y-10,W,H,'#9ca3af','#9ca3af','',false)); } if(busLbl){ f.appendChild(mk(COL.c6,y-10,W,H,getPinColor(busLbl),'#9ca3af',busLbl,true,isSmall(busLbl))); }else{ f.appendChild(mk(COL.c6,y-10,W,H,'#9ca3af','#9ca3af','',false)); } R.appendChild(f); };
 ...
 const left=(row,tl,tc,dl)=>{ const y=30+row*RH; const f=document.createDocumentFragment(); if(tl){ f.appendChild(mk(COL.c1,y-10,W,H,tc,'#9ca3af',tl,true)); }else{ f.appendChild(mk(COL.c1,y-10,W,H,'#9ca3af','#9ca3af','',false)); } f.appendChild(mk(COL.c2,y-10,W,H,FC.DIGITAL,'#9ca3af',dl,true)); L.appendChild(f); };
 ...
 const right=(row,dl,tl,tc)=>{ const y=30+row*RH; const f=document.createDocumentFragment(); f.appendChild(mk(COL.c4,y-10,W,H,FC.DIGITAL,'#9ca3af',dl,true)); f.appendChild(mk(COL.c5,y-10,W,H,tc,'#9ca3af',tl,true,isSmall(tl))); R.appendChild(f); };
 if(!isS3){
 ...
 // Afficher le type de message si disponible
 ...
 const fieldsToWatch=['funcSelect','btnMode','btnPulseTiming','ledMode','potFilter','rtpEnabled2','rtpMsgType','rtpNote','rtpCc','rtpPc','rtpChan','rtpCcOn','rtpCcOff','rtpVel','rtpCcMin','rtpCcMax','rtpNoteMin','rtpNoteMax','rtpNoteVelFix','rtpNoteSweepAutoOffDelay','oscEnabled2','oscAddress','oscFormat','dbgEnabled','dbgHeader'];
 ...
 // Sauvegarder dans pcfg et mettre à jour la liste
 if(cur){ 
 pcfg[cur]=readCfg(); 
 updatePinsList(); 
 updateBusVisuals();
