...
.f input,.f select,.f textarea{width:100%;padding:12px;border:1px solid #d1d5db;border-radius:6px;font-size:14px}
.f input:focus,.f select:focus,.f textarea:focus{outline:none;border-color:var(--cd);box-shadow:0 0 0 3px rgba(59,130,246,.1)}
...
.pl{display:flex;gap:20px;align-items:flex-start}.lp{flex:0 0 30%;min-width:300px}.rp{flex:1 1 auto;position:relative;left:75px}
...
.b{display:block;width:100%;min-width:360px;height:280px;max-width:100%;border:1px solid var(--bd);border-radius:8px;background:var(--bg);overflow:visible}
...
.svg-val{font-size:9px;fill:var(--tx);dominant-baseline:middle;pointer-events:none;user-select:none}
.svg-led{fill:#10b981;pointer-events:none}
...
.dbg-console{margin-top:10px;padding:10px;background:#111827;color:#e5e7eb;font-size:12px;line-height:1.35;max-height:240px;overflow:auto;border-radius:6px;font-family:ui-monospace,SFMono-Regular,Menlo,monospace;white-space:pre-wrap;word-break:break-word;border:1px solid #374151}
</style>
...
<body>
 ...
 <div class="f"><h3>Serveur</h3><form id="mdns"><div class="f"><label for="mdnsName">Nom du serveur</label><input type="text" id="mdnsName" placeholder="nidmi" required><div class="hint"><small>Ce nom sera utilisé pour l'AP Wi‑Fi et http://nom.local</small></div></div><button type="submit" class="btn">Enregistrer</button><div class="hint" id="mdnsMsg"></div></form></div>
 <div class="f"><h3>OSC</h3><form id="osc"><div class="f"><label><input type="checkbox" id="oscBroadcast"> Mode broadcast</label><div class="hint">Diffusion vers tous les appareils du réseau</div></div><div class="f" id="oscLinksRow"><label>Liens de diffusion</label><label class="chk"><input type="checkbox" id="oscLinkAp"> Point d’accès (192.168.4.255)</label><label class="chk"><input type="checkbox" id="oscLinkSta"> Réseau WiFi rejoint</label><label class="chk" id="oscLinkUsbRow" style="display:none"><input type="checkbox" id="oscLinkUsb"> Câble USB (192.168.7.255)</label><div class="hint">Plusieurs liens possibles : le message part sur chacun.</div></div><div class="f" id="oscIpRow"><label for="oscIp">Adresse IP</label><input type="text" id="oscIp" placeholder="192.168.1.100" pattern="^(?:[0-9]{1,3}\.){3}[0-9]{1,3}$"></div><div class="f"><label for="oscPort">Port</label><input type="number" id="oscPort" value="8000" min="1024" max="65535" required></div><button type="submit" class="btn">Enregistrer</button><div class="hint" id="oscMsg"></div></form></div>
<div class="f"><h3>OSC sortie</h3><div class="r switch"><input type="checkbox" id="oscEnabled2"><label for="oscEnabled2">Activer la sortie OSC</label></div><div class="hint" id="oscOutputMsg" style="margin-top:8px"></div><div class="hint"><small>Enregistrement immédiat sur l'appareil ; pas besoin du bouton Enregistrer tout (onglet Pins).</small></div></div>
<div class="f"><h3>Interfaces MIDI</h3><div class="r switch"><input type="checkbox" id="rtpMidiEnabled"><label for="rtpMidiEnabled">RTP-MIDI</label></div><div class="r switch"><input type="checkbox" id="usbMidiEnabled" disabled title="Réglage compile-time"><label for="usbMidiEnabled">USB-MIDI (build)</label></div><div class="r"><label>Statut USB-MIDI:</label><span id="usbMidiStatus">-</span></div><div class="hint"><small>USB-MIDI : activer dans le firmware (<code>UsbMidiManager.h</code> → <code>NIDMI_USB_MIDI_ENABLED_AT_COMPILE_TIME</code>), pas depuis l’interface.</small></div></div>
<div class="f"><h3>Firmware (OTA)</h3><p class="hint">Mettre à jour le firmware ou changer de variante par Wi-Fi, sans câble. Choisir l’image <code>.bin</code> (image app, pas <code>.merged.bin</code>) ; l’appareil redémarre dessus. Nécessite une partition OTA (build <code>--ota</code>).</p><form id="ota"><div class="f"><label for="otaFile">Image firmware (.bin)</label><input type="file" id="otaFile" accept=".bin" required></div><button type="submit" class="btn">Flasher</button><div class="hint"><small>Variantes : <code>usbmidi-off</code> (série stable) / <code>usbmidi-on</code> (MIDI USB).</small></div><progress id="otaProgress" value="0" max="100" style="width:100%;display:none;margin-top:8px"></progress><div class="hint" id="otaMsg"></div></form></div>
<div class="f" id="webDebugConsoleSection" style="display:none"><h3>Console debug</h3><p class="hint">Logs firmware via WebSocket (ESP32-S3). Utile lorsque le port série USB n’est pas disponible (ex. USB-MIDI). Optionnel : activer la réception ci-dessous.</p><div class="r switch"><input type="checkbox" id="webDebugConsoleEnabled"><label for="webDebugConsoleEnabled">Recevoir les logs</label></div><button type="button" class="btn" id="webDebugConsoleClear" style="margin-top:10px;background:#6b7280;">Effacer l’affichage</button><pre id="webDebugConsoleOut" class="dbg-console"></pre></div>
...
<div class="f" id="systemSettingsSection" style="display:none"><h3>Paramètres système</h3><form id="system"><div class="f" id="touchRow"><label><input type="checkbox" id="touchEnabled"> Activer le support Touch (ESP32-S3)</label><div class="hint">Active ou désactive l'utilisation des pins touch. Nécessite un rechargement des composants.</div></div><div class="f"><label for="componentsPerCycle">Composants par cycle :</label> <input type="number" id="componentsPerCycle" min="1" max="64" style="width:80px"><div class="hint">Nombre de composants lus a chaque cycle de 10 ms (defaut 4). Avec N composants configures, chacun est relu toutes les ceil(N / valeur) x 10 ms : monter la valeur reduit la latence de detection, au prix de plus de travail par cycle.</div></div><button type="submit" class="btn">Enregistrer</button><div class="hint" id="systemMsg"></div></form><div style="margin-top:30px;padding-top:20px;border-top:2px solid #ef4444"><h4 style="color:#ef4444;margin-bottom:10px">Zone de danger</h4><p style="color:#6b7280;font-size:14px;margin-bottom:15px">⚠️ Attention : Cela supprimera toutes les configurations (pins, OSC, WiFi, etc.) et réinitialisera complètement le système.</p><button type="button" id="clearNvsBtn" class="btn" style="background:#ef4444">Effacer toute la NVS</button><button type="button" id="resetDeviceBtn" class="btn" style="background:#6b7280;margin-top:10px;">Reset appareil</button><div class="hint" id="clearNvsMsg"></div></div></div>
</div>
...
<div class="r"><label>Format:</label><select id="oscFormat"><option value="float">Float (0-1)</option><option value="midi">MIDI (3 int)</option><option value="raw">Raw</option></select></div>
 ...
 <h3>Configuration HC4067</h3>
 <form id="muxForm">
 <div id="muxModalFormCard"></div>
 <button type="submit" class="btn">Enregistrer</button>
 <button type="button" class="btn" style="background:#6b7280;" onclick="hideMuxForm()">Annuler</button>
