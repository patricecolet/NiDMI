...
Les 3 personnes peuvent prendre chacune une step (ou plusieurs), en parallèle quand c’est raisonnable.
...
Décider si on fait :
plutôt des cherry-pick depuis feature/c3-partition-no-spiffs
ou une ré-implémentation guidée par diff (utile si l’historique de la feature est un peu “brouillon”).
...
Éventuelles références dans scripts/nidmi.sh, docs, system-config.js, etc.
Tâches
Définir clairement :
...
Assigné : à décider (par ex. la personne la plus à l’aise avec les outils de build / flash).
