 ...
 *{margin:0;padding:0;box-sizing:border-box}
 ...
 if ($('#muxOscBase')) {
 const selectedMuxId = idSel && idSel.value !== '' ? idSel.value : '0';
 $('#muxOscBase').value = '/mux' + selectedMuxId;
 }
 }
 ...
 const availableSig = analogPins.filter(p => !usedGpiosForSigAndEn.has(p.gpio));
 const sigSel = $('#muxSig');
 if (sigSel) {
 ...
 if ($('#muxOscBase') && idSel.value !== '') {
 $('#muxOscBase').value = '/mux' + idSel.value;
 }
 populateMuxPinSelects();
 });
 }
